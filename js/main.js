...
function populateContent(data) {
  ...
  if (data.ctaBand.form) {
    ctaBand.innerHTML = `
      <div>
        <h2 style="margin:0 0 6px">${data.ctaBand.heading}</h2>
        <p style="margin:0;color:#E5E7EB">${data.ctaBand.description}</p>
      </div>
      <form id="contact-form" action="${data.ctaBand.form.action}" method="POST" class="contact-form">
        ${data.ctaBand.form.fields.map(field => {
      ...
      return `<input type="${field.type}" name="${field.name}" placeholder="${field.placeholder}" ${field.required ? 'required' : ''}>`;
        ...
        <button type="submit" class="cta">${data.ctaBand.form.submitText}</button>
      </form>
    `;

    const form = document.getElementById('contact-form');
    form.addEventListener('submit', handleFormSubmit);
  } else if (data.ctaBand.cta) {
    ctaBand.innerHTML = `
      <div>
        <h2 style="margin:0 0 6px">${data.ctaBand.heading}</h2>
        <p style="margin:0;color:#E5E7EB">${data.ctaBand.description}</p>
      </div>
      <a class="cta" href="${data.ctaBand.cta.href}" role="button" aria-label="Start audit booking">${data.ctaBand.cta.text}</a>
    `;
  }
